...
class _SplashScreenState extends State<SplashScreen>
  ...
  void initState() {
    ...
    // 2. Change the Timer to trigger the auth check instead of hardcoding onboarding
    Timer(const Duration(seconds: 3), _navigateBasedOnAuth);
  }

  // 3. Restore the Auth checking logic
  Future<void> _navigateBasedOnAuth() async {
    User? currentUser = _auth.currentUser;

    // If no user is logged in, go to Onboarding
    if (currentUser == null) {
      _pushWithAnimation(AppRoutes.onboarding, isNamed: true);
      return;
    }

    // If user is logged in, check if their profile exists in Firestore
    try {
      ...
      if (userDoc.exists) {
        ...
        // User is authenticated but profile is missing, go to SignUp
        _pushWithAnimation(
          SignUpScreen(
