...
// UPDATED: Use the official firebase_vertexai package
...
class _VirtualTryOnScreenState extends State<VirtualTryOnScreen> {
  ...
  Future<void> _initializeGemini() async {
    ...
    _model = FirebaseVertexAI.instance.generativeModel(
      model: 'gemini-2.5-flash-image', // The required model for image editing tasks
      generationConfig: GenerationConfig(
        // FIXED: Reverted to the plural ResponseModalities as required by firebase_vertexai
  ...
  Future<void> _generateVirtualTryOn() async {
    ...
    try {
      ...
      // Note: Depending on your exact vertex_ai package version, 
      // 'InlineDataPart' might need to be 'DataPart'. 
...
""";
      ...
      // Handle the output. You may need to adapt this property 
      // depending on the exact vertex ai version response structure.
      if (response.text == null && response.candidates.isNotEmpty) {
         // Some versions return images nested inside candidates.parts
         final firstPart = response.candidates.first.content.parts.first;
         if (firstPart is InlineDataPart) {
           setState(() {
             _resultImage = firstPart.bytes;
           });
           return;
         }
      }

      // Fallback to your original logic if inlineDataParts is available in your SDK version
      try {
        // We use dynamic here to gracefully handle if the SDK version doesn't expose inlineDataParts natively
        final dynamic dynamicResponse = response; 
        if (dynamicResponse.inlineDataParts.isNotEmpty) {
          final imageBytes = dynamicResponse.inlineDataParts.first.bytes;
          setState(() {
         ...
         final text = response.text ?? "Response was empty.";
          ...
          _showError(
            "No image was generated. Try rephrasing or using fewer garments.",
          );
      }
...
await _firestore
    ...
    .add({
      'imageUrl': imageUrl,
      'timestamp': FieldValue.serverTimestamp(),
      'filename': 'try_on_result_${DateTime.now().millisecondsSinceEpoch}.jpg',
      'type': 'try_on_result',
      'garmentsCount': _garmentImages.length,
    });

_showError("Try-on image saved successfully to History and Dress Room!");
